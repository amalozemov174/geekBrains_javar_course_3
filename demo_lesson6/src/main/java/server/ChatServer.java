        ...
        try(ServerSocket serverSocket = new ServerSocket(8189);) {
            while(true){
               // System.out.println("server waiting connection from client...");
                ChatLogger.logLogger.info("server waiting connection from client...");
                final Socket socket = serverSocket.accept();
                ChatLogger.logLogger.info("Client connected...");
                //System.out.println("Client connected...");
                new ClientHandler(socket, this, authService, executorService);
            }
        } catch (IOException e) {
            ChatLogger.logLogger.info("Server error" + e.getMessage());
